...
dv13s = [0;norm(0 - speed_leader_3)];
dv23s = [0;norm(0 - speed_leader_3)];
dv43s = [0;norm(0 - speed_leader_3)];
...
%     end
    ...
    dv13s = [dv13s, [t*dt;norm(v(:,1)-v(:,3))]];
    dv23s = [dv23s, [t*dt;norm(v(:,2)-v(:,3))]];
    dv43s = [dv43s, [t*dt;norm(v(:,4)-v(:,3))]];

%     t0=steps* 1/10;
% %     t0=0;
%     if t>=t0
%         dv13s = [dv13s,[(t-t0)*dt;norm(v(:,1) - speed_leader_3*[cos(speed_leader_3_alpha);sin(speed_leader_3_alpha)])]];
%         dv23s = [dv23s,[(t-t0)*dt;norm(v(:,2) - speed_leader_3*[cos(speed_leader_3_alpha);sin(speed_leader_3_alpha)])]];
%         dv43s = [dv43s,[(t-t0)*dt;norm(v(:,4) - speed_leader_3*[cos(speed_leader_3_alpha);sin(speed_leader_3_alpha)])]];
%     end
end
...
for i=steps/10 : steps
    dv13s(2,i-steps/10 + 1) = dv13s(2,i);
    dv23s(2,i-steps/10 + 1) = dv23s(2,i);
    dv43s(2,i-steps/10 + 1) = dv43s(2,i);
end
% 
% for i=steps - steps/10 : steps
%     dv13s(2,i) = dv13s(2,steps);
%     dv23s(2,i) = dv23s(2,steps);
%     dv43s(2,i) = dv43s(2,steps);
% end

plot(dv13s(1,:),dv13s(2,:),'LineWidth',1.5);
hold on;
plot(dv23s(1,:),dv23s(2,:),'LineWidth',1.5);
plot(dv43s(1,:),dv43s(2,:),'LineWidth',1.5);
grid on;
legend('agent 1','agent 2','agent 4');
xlabel('t/s'); ylabel('║Vi-Vleader║');
