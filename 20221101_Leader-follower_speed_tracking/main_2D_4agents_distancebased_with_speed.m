...
%
% Used the following paper for velocity compensation:
% <Distance-based Formation Control with a Single Moving Leader>
% and it worked, the formation can follow the leader, but cannot turn
%
% 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
close all; clear all;
...
x4_ini = q4d+ 0.5*[rand()-0.5;rand()-0.5];
...
speed_leader_3 = 0.2;
speed_leader_1_alpha = 20/180*pi;
omega_leader_1 = 0.9;
...
for t=1:steps
    ...
    for i=1:4
        for j=1:4
            ...
            a_curij = speed_track(x(:,i),x(:,j),dijd(i,j));
            v_fs(:,i) = v_fs(:,i)+a_curij*dt;
            v(:,i) = v(:,i)+force(x(:,i),x(:,j),dijd(i,j));
        end
        v(:,i) = v(:,i) + v_fs(:,i);
    end 
    v(:,3) = [0.09;0.046];
    v(:,3) = speed_leader_3*[cos(speed_leader_1_alpha);sin(speed_leader_1_alpha)];
...
function f = force(x1,x2,dij)
    f = (x2-x1)/norm(x2-x1)*(norm(x2-x1)^2 - dij^2);
end

function a = speed_track(x1,x2,dij)
    a = (x2-x1)/norm(x2-x1)*(norm(x2-x1)^2 - dij^2) * 10;
end
