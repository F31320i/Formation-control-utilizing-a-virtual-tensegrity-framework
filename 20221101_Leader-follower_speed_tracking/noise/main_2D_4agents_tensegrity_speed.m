...
for t=1:steps
    ...
    for i=1:4
        for j=1:4
            ...
            x_observed_j = x(:,j);
            if t > steps*0.0 && t < steps*0.75
                x_observed_j = x(:,j) + ((steps*0.75-t)/(steps*0.75))*5*[rand()-0.5;rand()-0.5];
            end
            
            if omega(i,j)>0 % strut
                v(:,i) = v(:,i)+force_strut(x(:,i),x_observed_j,omega(i,j),dijd(i,j));
            elseif omega(i,j)<0 % cable
                v(:,i) = v(:,i)+force_cable(x(:,i),x_observed_j,omega(i,j),dijd(i,j));
            end
        end
    end 
