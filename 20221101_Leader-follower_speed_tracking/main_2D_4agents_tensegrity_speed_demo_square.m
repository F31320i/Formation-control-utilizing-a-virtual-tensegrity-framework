...
%  Time: 2022.8.19
%  This is only a demo
%  Given the desired formation is a square
...
speed_leader_3 = sqrt(2);
speed_leader_3_alpha = 10 + 66*rand(); % deg
speed_leader_3_alpha = speed_leader_3_alpha/180*pi;
%% omega calculation
qxd = [q1d(1);q2d(1);q3d(1);q4d(1)];

k = sqrt(-sqrt(2)/2*speed_leader_3/([1 1 -3 1]*qxd));
D = k*[1;1;-3;1];
omega = D*D';
...
dijd = dijd+dijd';
%%
x1_ini = q1d+ 3*[rand()-0.5;rand()-0.5];
x2_ini = q2d+ 3*[rand()-0.5;rand()-0.5];
x3_ini = q3d+ 3*[rand()-0.5;rand()-0.5];
