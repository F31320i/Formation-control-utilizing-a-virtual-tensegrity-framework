...
% dv1s = [0;norm(0 - speed_leader_3*[cos(speed_leader_3_alpha);sin(speed_leader_3_alpha)])];
% dv2s = [0;norm(0 - speed_leader_3*[cos(speed_leader_3_alpha);sin(speed_leader_3_alpha)])];
% dv4s = [0;norm(0 - speed_leader_3*[cos(speed_leader_3_alpha);sin(speed_leader_3_alpha)])];
dv1s =[];
dv2s =[];
dv4s =[];
...
for t=1:steps
    ...
    for i=1:4
        for j=1:4
            ...
            if omega(i,j)>0 % strut
                force(:,i) = force(:,i)+force_strut(x(:,i),x(:,j),omega(i,j),dijd(i,j));
            elseif omega(i,j)<0 % cable
                force(:,i) = force(:,i)+force_cable(x(:,i),x(:,j),omega(i,j),dijd(i,j));
            end
        end
    end
    ...
    for i = 1:4
        ...
        if norm(force(:,i)) > force_threshold
            force(:,i) = force(:,i)/norm(force(:,i))*force_threshold;
        end
        v(:,i) = force(:,i) + v(:,i);
    end
...
%         dv4s = [dv4s,[(t-t0)*dt;norm(v(:,4) - speed_leader_3*[cos(speed_leader_3_alpha);sin(speed_leader_3_alpha)])]];
%     end
        dv1s = [dv1s,[(t)*dt;norm(v(:,1) - speed_leader_3*[cos(speed_leader_3_alpha);sin(speed_leader_3_alpha)])]];
        dv2s = [dv2s,[(t)*dt;norm(v(:,2) - speed_leader_3*[cos(speed_leader_3_alpha);sin(speed_leader_3_alpha)])]];
        dv4s = [dv4s,[(t)*dt;norm(v(:,4) - speed_leader_3*[cos(speed_leader_3_alpha);sin(speed_leader_3_alpha)])]];
...
plot(dv4s(1,:),dv4s(2,:),'LineWidth',1.5);
grid on;
legend('agent 1', 'agent 2', 'agent 4');
...
for i=1:4
    ...
    legend([H([1 2 3 4]), G, K] ,'agent 1','agent 2','agent 3 (leader)','agent 4','initial configuration','final configuration');
    grid on;
