...
% q1d = [0;0] + 5*[rand()-0.5;rand()-0.5];
% q2d = [1;0] + 5*[rand()-0.5;rand()-0.5];
% q3d = [1;1] + 5*[rand()-0.5;rand()-0.5];
% q4d = [0;1] + 5*[rand()-0.5;rand()-0.5];
    ...
    for i=1:4
        for j=1:4
            ...
            elseif omega(i,j)<0 % cable
                v_ini = v_ini+ 0.5*1/(expd_c+1)*(-omega(i,j)/(dijd(i,j)^(2*expd_c)))*(norm(x_now(:,i) - x_now(:,j))^2)^(expd_c+1);
            end
        end
    end 
vs = [0;v_ini];
